...
struct AgentLoopTests {
  ...
  @Test func requestStopReturnsFinalDurableState() async throws {
    let store = SimpleLoopStore(durableState: .init(acceptedCount: 0, needsInference: false))
    let behavior = SimpleLoopBehavior(store: store)
    let loop = try await AgentLoop(behavior: behavior, initialState: behavior.loadState())

    let startTask = Task { try await loop.start() }

    await loop.send(.accept)
    await loop.requestStop()

    let finalState = try await startTask.value
    #expect(finalState.acceptedCount == 1)
    #expect(finalState.needsInference == false)

    let durableState = await store.loadState()
    #expect(durableState == finalState)
  }
}
...
private actor SimpleLoopStore {
  ...
  func persist(_ newState: FailureCleanupState) -> FailureCleanupState {
    durableState = newState
    return newState
  }
}

private struct SimpleLoopBehavior: AgentBehavior {
  typealias State = FailureCleanupState
  typealias PersistenceDiff = FailureCleanupState
  typealias StreamAction = String
  typealias ExternalAction = FailureCleanupAction
  ...
  func infer(
    ...
    AssistantMessage(provider: .openai, model: "unused", content: [], usage: nil, stopReason: .stop, errorMessage: nil, timestamp: Date())
  }

  func persistAssistantEntry(_: AssistantMessage, state _: inout FailureCleanupState) {}
  func nextToolCall(state _: FailureCleanupState) -> ToolCall? {
    nil
  }
  ...
  func persistToolResult(_: String, for _: ToolCall, state _: inout FailureCleanupState) {}
  func shouldCompact(state _: FailureCleanupState) -> Bool {
    false
  }
