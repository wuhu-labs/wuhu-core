...
struct InMemoryFileIOToolTests {
  ...
  private func tools(fileIO: InMemoryFileIO) -> [String: AnyAgentTool] {
    Dictionary(
      uniqueKeysWithValues:
      WuhuTools.codingAgentTools(cwd: cwd, fileIO: fileIO)
        .map { ($0.tool.name, $0) },
    )
  }
  ...
  @Test func readFileNotFound() async throws {
    let io = makeIO()
    let t = try #require(tools(fileIO: io)["read"])
  ...
  @Test func writeCreatesParentDirs() async throws {
    let io = makeIO()
    let t = try #require(tools(fileIO: io)["write"])
  ...
  @Test func editExactMatch() async throws {
    ...
    let t = try #require(tools(fileIO: io)["edit"])
  ...
  @Test func lsEmptyDirectory() async throws {
    ...
    let t = try #require(tools(fileIO: io)["ls"])
  ...
  @Test func lsLimit() async throws {
    ...
    // Only 3 entries before the notice.
  ...
  @Test func findGlobMatching() async throws {
    ...
    let t = try #require(tools(fileIO: io)["find"])
    let result = try await t.execute(
      toolCallId: "f1",
  ...
  @Test func grepRegex() async throws {
    ...
    let t = try #require(tools(fileIO: io)["grep"])
    let result = try await t.execute(
      toolCallId: "g1",
  ...
  @Test func grepLiteral() async throws {
    ...
    // Literal: should not match "foo.bar" through regex semantics.
    // (Note: "foo*bar" literally matches only "foo*bar".)
  ...
  @Test func grepLimit() async throws {
    ...
    // Only 5 match lines before the notice.
